export class TicTacToe {
   ...
   * @returns { VMap: string[][], HMap_90: string[][] } An object containing the vertical map (VMap) and the 90-degree
   * rotated map (HMap_90).
   */
  handleV(HMap: string[][], VMap: string[][] = []): { VMap: string[][], HMap_90: string[][] } {
    const HMap_90: string[][] = HMap[0].map((_val, index) => HMap.map(row => row[index]).reverse());
    HMap_90.forEach(val => {
  ...
  initMatrix() {
    const HMap: string[][] = this.handleH([]);
    ...
    const _mapArray: Set<string> = new Set([
      ...this.winMap,
      ...VMap,
      ...diagonalsH,
      ...diagonalsV
    ...
    // @ts-ignore
    this.winMap = Array.from(_mapArray, JSON.parse);
